...
// Manual state for dropdown
const openDocs = ref(false)
const manualDocsToggle = ref(null)
...
<template>
  <div>
    <aside :class="['sidebar', { open: isOpen }]">
      ...
      <nav class="menu">
        <ul>
          ...
          <li v-if="['Admin', 'Admin Staff'].includes(user.role)">
          ...
          <li v-if="user.role === 'Teacher'">
            <a @click="go('teacher.my-profile')" :class="['nav-link', { active: isActive('teacher.my-profile') }]">
              <i class="bi bi-person-circle"></i> Profile
            </a>
          </li>

          <li v-if="['Admin', 'Admin Staff', 'Teacher'].includes(user.role)">
    ...
    <transition name="fade">
      <div v-if="showLogoutModal" class="ux-modal" @click="closeOnBackdrop" @keyup.esc="showLogoutModal = false" tabindex="-1" role="dialog">
        <transition name="pop">
          <div class="ux-modal-card" role="document">
            ...
            <div class="ux-modal-footer">
              ...
              <button class="btn btn-danger" @click="doLogout" :disabled="isLoggingOut">
...
<style scoped>
.sidebar { width: 210px; height: 100vh; background: linear-gradient(to bottom, #12172b, #1a1f3a); color: white; display: flex; flex-direction: column; justify-content: space-between; position: fixed; top: 0; left: 0; z-index: 1000; padding: 20px 0; overflow-y: auto; scrollbar-width: none; -ms-overflow-style: none; }
...
.nav-link { display: flex; align-items: center; padding: 10px 16px; gap: 10px; font-size: 15px; color: white; text-decoration: none; transition: background 0.2s, color 0.2s; border-radius: 12px; }
...
.dropdown-link { display: flex; align-items: center; gap: 8px; padding: 6px 16px; font-size: 14px; color: #ccc; text-decoration: none; transition: background .2s, color .2s; border-radius: 12px; }
...
.section-title { font-size: 13px; font-weight: 600; color: #a1a1a1; margin-top: 10px; padding-left: 16px; text-transform: uppercase; }
.logout { margin-top: 20px; padding: 12px 16px; font-size: 14px; color: white; cursor: pointer; display: flex; align-items: center; gap: 10px; }
.logout:hover { background: rgba(255,255,255,0.1); }
.overlay { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.3); z-index: 900; }
...
.ux-modal { position: fixed; inset: 0; background: rgba(17,24,39,.55); display: grid; place-items: center; z-index: 2000; }
